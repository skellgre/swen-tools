import serial
import time
from exceptions.exceptions import CommandFailedError, FlashScriptError, PortNotFoundError
...
class SerialCommandStrategy(ABC):
    ...
    @abstractmethod
    def execute(self, ser: serial.Serial, command: bytes, logger: Logger, expected_response: bytes, timeout: int):
        pass
...
class BasicSerialCommand(SerialCommandStrategy):
    ...
    def execute(self, ser: serial.Serial, command: bytes, logger: Logger, expected_response: bytes, timeout: int):
        ...
        response = ser.read(ser.in_waiting or 1024).decode('utf-8', errors='replace').strip()

        if expected_response not in response:
            logger.warning(f"Unexpected response. Expected: {expected_response}, Actual: {response}")
        else:
...
class SerialCommandExecutor:
    ...
    def execute(self, ser: serial.Serial, command: bytes, logger: Logger, expected_response: bytes, timeout: int = 0):
        return self.strategy.execute(ser, command, logger, expected_response, timeout)


def serial_command(ser: serial.Serial, cmd, logger: Logger, expected_response = b"\r", timeout = 5):
    assert ser.is_open
    try:
        ...
        actual_response = ser.read(ser.in_waiting or 1024).decode('utf-8', errors='replace').strip()
        if expected_response != actual_response:
            logger.warning(f"Failed to get expected response. Exepected:{expected_response}. Actual:{actual_response}")
        else:
            logger.success(f"Got expected reponse from executing command: {expected_response}")
...
def find_active_ttyUSB_port(prompt, max_ports, logger: Logger, enter_checks: int = 3) -> int:
        ...
        try:
            with serial.Serial(port, **SERIAL_CONFIG) as ser:
                for _ in range(enter_checks):  # Check multiple times per port
                    ...
                    if any(p in output for p in prompt):  # Check if output contains any prompt
...
def execute_commands_on_ttyUSB_port(port_num, commands: list[str], expected_response, error_response, logger: Logger):
    """
    ...
    try:
        with serial.Serial(port, **SERIAL_CONFIG) as ser:
            ...
            for cmd in commands:
                ...
                print("output: ", output)
                if filtered_output in error_response or not expected_response in filtered_output:
